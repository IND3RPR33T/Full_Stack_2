// OpenAI Service - Uses backend API for secure API key handling
...
export const initializeOpenAI = () => {
    console.log('Initializing OpenAI service (using backend)...');
...
export const sendMessage = async (message) => {
    ...
    try {
        ...
        if (error.message?.includes('fetch') || error.message?.includes('Failed to fetch')) {
            ...
            throw new Error('OpenAI API key not configured on backend server. Please check server/.env file.');
...
// Check if OpenAI is available (always true since we use backend)
